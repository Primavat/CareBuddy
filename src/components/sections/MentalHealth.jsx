...
const MentalHealth = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto font-sans">
      <div className="mb-14 pb-6 border-b border-border">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        {metrics.map((m, i) => (
          <motion.div
            ...
            className="bg-card-bg p-8 rounded-[3rem] border border-border flex items-center gap-6 group hover:border-primary transition-all shadow-sm"
          >
            <div className={`w-14 h-14 bg-bg-main ${m.color} rounded-2xl flex items-center justify-center border border-border group-hover:bg-primary group-hover:text-white transition-all`}>
              <m.icon size={28} />
            </div>
            <div>
              <h4 className="text-[10px] font-black text-gray-400 dark:text-text-dim uppercase tracking-widest">{m.name}</h4>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-bg-main border-2 border-dashed border-border rounded-[3rem] p-10 flex flex-col items-center justify-center text-center">
            <div className="w-16 h-16 bg-card-bg shadow-xl rounded-[2rem] flex items-center justify-center mb-6 text-primary border border-border"><Sun size={32} /></div>
            ...
            <div className="bg-amber-100 dark:bg-amber-950/50 text-amber-700 dark:text-amber-200 px-4 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest animate-pulse shadow-sm">Premium Feature - Launching Soon</div>
        </div>
      </div>
    </div>
  );
