...
const Journal = ({ journalEntries, setJournalEntries }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-14 pb-4 border-b border-gray-100 dark:border-border">
      ...
      <div className="bg-card-bg p-6 rounded-3xl border border-border shadow-sm mb-10">
        <textarea 
          value={content}
          onChange={(e) => setContent(e.target.value)}
          placeholder="How are you feeling today? (Limit: 1,000 words)..."
          className="w-full min-h-[200px] p-4 text-secondary font-semibold bg-gray-50 dark:bg-bg-main border border-border rounded-2xl focus:border-primary outline-none transition-colors"
        />
      ...
      <div className="flex flex-col gap-6">
        <AnimatePresence>
          {journalEntries.length === 0 ? (
            <div className="text-center py-20 bg-gray-50 dark:bg-bg-main rounded-3xl border-2 border-dashed border-border text-text-dim font-bold">
            ...
            journalEntries.map((entry) => (
              <motion.div
                ...
                className={`bg-card-bg p-6 rounded-3xl border border-border shadow-sm relative transition-all ${entry.starred ? 'border-amber-400 bg-amber-50/20 dark:bg-amber-950/20 dark:border-amber-500' : ''}`}
              >
                ...
                {editingId === entry.id ? (
                  <textarea 
                    ...
                    className="w-full min-h-[150px] p-3 border border-border rounded-2xl text-secondary font-semibold bg-card-bg focus:border-primary outline-none"
                  />
                ) : (
                  <p className="text-secondary font-bold leading-relaxed whitespace-pre-wrap mb-6">{entry.text}</p>
                )}

                <div className="flex gap-4 pt-4 border-t border-gray-100 dark:border-border mt-4">
                  {editingId === entry.id ? (
